...
const NewUser = () => {
  ...
  const usernameConfig = {
    ...
    min: {
      value: 5,
      message: "Dit brugernavn er for kort - mindst 5 karakterer",
    },
    max: {
      value: 20,
      message: "Dit brugernavn er for langt - højest 20 karakterer",
    },
  };
  const emailConfig = {
    ...
    pattern: {
      value: /^\S+@\S+$/i,
      message: "Din email-adresse er ikke gyldig",
    },
  };
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <input
        ...
        name="username"
        ref={register(usernameConfig)}
      />
      <input
        type="text"
        placeholder="email"
        name="email"
        ref={register(emailConfig)}
      />
      <input type="submit" />
    </form>
  );
